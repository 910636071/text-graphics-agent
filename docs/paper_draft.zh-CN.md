...
这种分离是双向的：人类侧的原始意图不能绕过 Intent Firewall 直接成为任务权威；AI 侧的生成输出不能绕过约束检查器直接成为可信状态。

同一原则也可以扩展到 LLM 游戏。模型可以叙述已批准事实、润色对白或提出面向玩家的文本，但不应直接编写 canonical game state。游戏状态应由确定性规则、已验证记录或明确的人类批准转换产生。本文草稿不评测游戏；这里只声明共享的权威边界。
